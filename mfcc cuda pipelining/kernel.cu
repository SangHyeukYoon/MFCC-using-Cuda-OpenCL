...
{
	const int id = blockDim.x * blockIdx.x + threadIdx.x + offset;
	...
	short* signal, float* frames)
{
	...
	const int frameNum = blockIdx.x + offset;
...
{
	const int id = threadIdx.x;
	const int blockId = blockIdx.x + offset;
	...
	float* mfcc)
{
	...
	if (id < halfSize)
	{
		scratch[id] = mfcc[id_t * numCeps + blockId] + mfcc[(id_t + halfSize) * numCeps + blockId];
	}
	else
	{
		scratch[id] = 0;
	...
	if (numFrames % 2 == 1)
	{
		if (id == 0)
		{
			scratch[id] += mfcc[(offset + numFrames - 1) * numCeps + blockId];
		}
	}
	...
	float mean = scratch[0] / float(numFrames) + 1.19209e-07;
	...
	//if (id < halfSize)
	//{
	//	mfcc[id_t * numCeps + blockId] -= mean;
	//	mfcc[(id_t + halfSize) * numCeps + blockId] -= mean;
	//
	//	if (id == 0)
	//	{
