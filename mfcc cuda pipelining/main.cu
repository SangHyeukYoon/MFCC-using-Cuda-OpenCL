...
int main()
{
    ...
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "cudaMemcpy failed! fbanks_val_d: %s\n", cudaGetErrorString(cudaStatus));
    ...
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "cudaMemcpy failed! fbanks_col_d: %s\n", cudaGetErrorString(cudaStatus));
    ...
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "cudaMemcpy failed! fbanks_row_d: %s\n", cudaGetErrorString(cudaStatus));
    ...
    int offset = 0;
    int stride = numFrames / nStreams;

    std::cout << stride << std::endl;

    int p2 = pow(2, ceil(log(stride / 2) / log(2)));

    for (int streamNum = 0; streamNum < nStreams; ++streamNum)
    {
        offset = streamNum * stride;

        cudaStatus = cudaMemcpyAsync(&signal_d[offset * frameStep], &signal_h[offset * frameStep], 
            ((stride - 1) * frameStep + frameLength) * sizeof(short), cudaMemcpyHostToDevice, streams[streamNum]);
        if (cudaStatus != cudaSuccess)
        {
        ...
        PreEmphasis <<< ceil((double(stride - 1) * frameStep + frameLength) / 1024.0), 1024, 0, streams[streamNum] >>> (PRE_EMPHASIS, signalLen, offset, signal_d, emphasised_d);

        cudaStatus = cudaGetLastError();
        if (cudaStatus != cudaSuccess)
        {
            fprintf(stderr, "PreProcessing launch failed: %s\n", cudaGetErrorString(cudaStatus));

            return 1;
        }

        PowerFFT << < stride, NFFT / 2, 512 * 3 * sizeof(float2), streams[streamNum] >> > (BIT_LENGTH, frameLength, frameStep, signalLen, offset, emphasised_d, powFrames_d);
        ...
        MelFilterBank_Sparse << < stride, NFILTER, 0, streams[streamNum] >> > (numFrames, fftResultLen, NFILTER, offset, 
            powFrames_d, filterBanks_d, fbanks_val_d, fbanks_col_d, fbanks_row_d);
        ...
        DCT << < stride, NFILTER, 0, streams[streamNum] >> > (numFrames, NFILTER, NUM_CEPS, CEP_LIFTER, offset, filterBanks_d, mfcc_d);
        ...
        MeanNorm << < NUM_CEPS, p2, p2 * sizeof(float), streams[streamNum] >> > (stride, NUM_CEPS, offset, mfcc_d);
        ...
        cudaStatus = cudaMemcpyAsync(&mfcc_h[offset * NUM_CEPS], &mfcc_d[offset * NUM_CEPS], 
            stride * NUM_CEPS * sizeof(float), cudaMemcpyDeviceToHost, streams[streamNum]);
        if (cudaStatus != cudaSuccess)
        {
            fprintf(stderr, "cudaMemcpy failed! mfcc_h: %s\n", cudaGetErrorString(cudaStatus));
